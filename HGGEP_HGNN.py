...
class HGGEP(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.bake > 0: # the number of augmented images.
            ...
            # self.log('bake_loss', bake_loss, on_epoch=True,
            #          prog_bar=True, logger=True)
        zinb_loss = 0
        if self.zinb > 0:
            if self.nb:
            ...
            # self.log('zinb_loss', zinb_loss, on_epoch=True,
            #          prog_bar=True, logger=True)
...
class shufflenet_v2(nn.Module):
    def __init__(self):
        super().__init__()
        shufflenet_v2 = models.shufflenet_v2_x0_5(pretrained=True)
        # features = list(self.shufflenet_v2.children())
        '''
        ...
        self.layer4 = shufflenet_v2.stage4
        self.layer5 = shufflenet_v2.conv5
        self.layer6 = shufflenet_v2.fc

    def forward(self, x):
        '''
        0 torch.Size([1, 24, 56, 56])
        1 torch.Size([1, 24, 28, 28])
        2 torch.Size([1, 48, 14, 14])
        3 torch.Size([1, 96, 7, 7])
        4 torch.Size([1, 192, 4, 4])
        5 torch.Size([1, 1024, 4, 4])
        '''
        x = self.layer1(x)
        print(x.shape)
        x = self.layer2(x)
